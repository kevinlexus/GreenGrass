...
public class ElementsHome {
	...
	public Elements merge(Elements detachedInstance) {
		...
		try {
			Elements result = (Elements) sessionFactory.getCurrentSession()
	...
	public Elements findById(java.math.BigDecimal id) {
		log.debug("getting Elements instance with id: " + id);
		try {
			Elements instance = (Elements) sessionFactory.getCurrentSession()
					.get("test1.Elements", id);
			if (instance == null) {
	...
	public List<Elements> findByExample(Elements instance) {
		log.debug("finding Elements instance by example");
		try {
			List<Elements> results = (List<Elements>) sessionFactory
					.getCurrentSession().createCriteria("test1.Elements")
